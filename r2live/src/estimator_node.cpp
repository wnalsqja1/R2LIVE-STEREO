...
int frame_count_1=0;
double trajSum1=0.0f;
double endToEndDist1=0.0f;
int firstFlag1=1;
double f_x1=0.0f;
double f_y1=0.0f;
double f_z1=0.0f;
...
std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> getMeasurements()
{
    ...
    while (true)
    {
        ...
        {
            // ROS_INFO("getMeausrements() :: imu_buf : %d feature_buf %d ", imu_buf.empty(), feature_buf.empty());
            return measurements;
        }
...
void imu_callback(const sensor_msgs::ImuConstPtr &_imu_msg)
{
    ...
    *imu_msg = *_imu_msg;
    ...
    double gravity_vector=1.0;
    if (1) // For LiVOX Avia built-in IMU
    {
        imu_msg->linear_acceleration.x *= gravity_vector;
        imu_msg->linear_acceleration.y *= gravity_vector;
        imu_msg->linear_acceleration.z *= gravity_vector;
    }

    // cout<<"linear accleration x :"<<imu_msg->linear_acceleration.x<<endl;
    // cout<<"linear accleration y :"<<imu_msg->linear_acceleration.y<<endl;
    // cout<<"linear accleration z :"<<imu_msg->linear_acceleration.z<<endl;
    ...
    estimator.accBuf.push(make_pair(t, acc));
    estimator.gyrBuf.push(make_pair(t, gyr));
    m_buf.unlock();
    con.notify_one();
...
void process()
{
    ...
    /*estimator_node_stereo.cpp 218 line VIO Start*/
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            *p_imu = *(estimator.m_fast_lio_instance->m_imu_process);
            ...
            /* feature 저장 ( Image frame 에 대한 )*/
            std::map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> image;
            for (unsigned int i = 0; i < img_msg->points.size(); i++)
            {   /* recive the img message from img_topic :: features */
            ...
            curTime = img_msg->header.stamp.toSec()+estimator.td;
            estimator.getIMUInterval(prevTime, curTime, accVector, gyrVector);

            if(!estimator.initFirstPoseFlag)
            ...
            {
                double t = imu_msg->header.stamp.toSec();
                double img_t = curTime;

                /* gravity accletation nG = 1.0 */
                if (t <= img_t)
                {/*IMU Measurements assignment*/
                    if (current_time < 0)
                        current_time = t;
            ...
            if (relo_msg != NULL)
            {
                ...
                Matrix3d relo_r = relo_q.toRotationMatrix(); /*quaternion to 3x3 matrix */
                int frame_index;
            ...
            if (imu_queue.size())
            {
                ...
                cout << "Start dt ( g_lio_state.last_update_tim - imu_queue.front.sec ) = " << start_dt << std::setprecision(2) << endl;
                cout << "Final dt ( cam_update_tim - imu_queue.back().sec)= " << end_dt << std::setprecision(2) << endl;
                ...
                if (g_camera_lidar_queue.m_if_have_lidar_data && (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR))
                {
                    *p_imu = *(estimator.m_fast_lio_instance->m_imu_process);
                    ...
                    {
                        ROS_INFO("estimator_node.cpp :: diff_vins_lio_t.norm > 1.0, So VIO subsystem restart!! ");
            ...
            /* VIO Stereo process 311 line start  ( estimator_node_stereo.cpp )*/
                ...
                )
            {
                ...
                // estimator.f_manager.initFramePoseByPnP(10, estimator.Ps, estimator.Rs, estimator.tic, estimator.ric);
                estimator.f_manager.triangulate(estimator.Ps, estimator.Rs, estimator.tic, estimator.ric);
                ...
                for (int iter_time = 0; iter_time < 2; iter_time++)
                {
                    ...
                    construct_camera_measure(win_idx, estimator, reppro_err_vec, J_mat_vec); /* camera measurement 생성  . . . */
            ...
            g_lio_state = state_before_esikf;
            t_s.tic();
            estimator.solve_image_pose_1(img_msg->header);
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                {
                    cout<<"estimator_node.cpp :: refine_vio_system function call"<<endl;
                    estimator.refine_vio_system(diff_vins_lio_q, diff_vins_lio_t);
                    diff_vins_lio_q.setIdentity();
                    diff_vins_lio_t.setZero();
                }
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
                time_vector.push_back(whole_t);
            printStatistics(estimator, whole_t, time_vector);
            ...
            {
                pubOdometry(estimator, header, frame_count_, trajSum, endToEndDist, firstFlag, f_x, f_y, f_z);
            }
            else
            {
                pub_LiDAR_Odometry(estimator, state_aft_integration, header, frame_count_1, trajSum1, endToEndDist1, firstFlag1, f_x1, f_y1, f_z1);
            }
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_image = nh.subscribe("/feature_tracker/feature", 20000, feature_callback,ros::TransportHints().tcpNoDelay()); /* Tcp latency late decerese */
