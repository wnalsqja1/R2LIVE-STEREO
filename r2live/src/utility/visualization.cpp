...
void printStatistics(const Estimator &estimator, double t ,vector<double> time_vector)
{
    ...
    double mean = sum_of_time / sum_of_calculation;


    double sum_of_time_square = 0;
    for( int i=0 ; i < sum_of_calculation; i++){
        sum_of_time_square = sum_of_time_square + (time_vector[i]-mean)*(time_vector[i]-mean);
    }

    double variance = sum_of_time_square/sum_of_calculation;
    
    ROS_DEBUG("--- current vio solver costs: %f ms", t);
    ROS_DEBUG("--- average of time %f ms", mean);
    ROS_DEBUG("--- standard deviation %f ms:", variance);
...
void pub_LiDAR_Odometry(const Estimator &estimator, const StatesGroup & state, const std_msgs::Header &header, 
                            int &frame_count_, double &trajSum, double &endToEndDist, int &firstFlag, double &f_x, double &f_y, double &f_z)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        nav_msgs::Odometry odometry;
        ...
        prev_x=odometry.pose.pose.position.x;
        prev_y=odometry.pose.pose.position.y;
        prev_z=odometry.pose.pose.position.z;
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header, int &frame_count_, double &trajSum, double &endToEndDist, int &firstFlag, double &f_x, double &f_y, double &f_z)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        /* for evaluation _ namutrajectory */
        
        // std::ofstream ofs("/home/minbum/ws_stereo/estimate.txt", std::ios::app);

        // ofs<<odometry.header.stamp<<" "<<odometry.pose.pose.position.x<<" "<<odometry.pose.pose.position.y<<" "<<
        // odometry.pose.pose.position.z<<" "<<odometry.pose.pose.orientation.x<<" "<<odometry.pose.pose.orientation.y<<" "<<odometry.pose.pose.orientation.z<<" "<<odometry.pose.pose.orientation.w<<std::endl;
        ...
        /* ================================= */
        /* for evaluation _ end to end error */

        // if ( frame_count_ > 100 ){
        //     trajSum +=    sqrt((odometry.pose.pose.position.x - prev_x)*(odometry.pose.pose.position.x - prev_x) 
        //     + (odometry.pose.pose.position.y - prev_y)*(odometry.pose.pose.position.y - prev_y)
        //     + (odometry.pose.pose.position.z - prev_z)*(odometry.pose.pose.position.z - prev_z));

        //     if(firstFlag && (odometry.pose.pose.position.x!=0.0) && (odometry.pose.pose.position.y!=0.0) && (odometry.pose.pose.position.z!=0.0))
        //     {
        //         firstFlag = 0;
        //         f_x = odometry.pose.pose.position.x;
        //         f_y = odometry.pose.pose.position.y;
        //         f_z = odometry.pose.pose.position.z; 
        //     }

        //     endToEndDist = sqrt((odometry.pose.pose.position.x - f_x)*(odometry.pose.pose.position.x - f_x) 
        //                 + (odometry.pose.pose.position.y - f_y)*(odometry.pose.pose.position.y - f_y)
        //                 + (odometry.pose.pose.position.z - f_z)*(odometry.pose.pose.position.z - f_z));

        //     std::cout.precision(5);
        //     std::cout << " " <<std::endl;
        //     std::cout <<"trajSum : "<< trajSum << std::endl;
        //     std::cout <<"end to end distance : "<< endToEndDist << std::endl;

        //     std::cout <<"Start position : "<< "("<< f_x << ", " << f_y << ", " << f_z << ")"<< std::endl;
        //     std::cout <<"End position : "<< "("<< odometry.pose.pose.position.x << ", " << odometry.pose.pose.position.y << ", " << odometry.pose.pose.position.z << ")"<< std::endl;

        //     std::cout <<"end to end error per total distance : "<< (endToEndDist / trajSum) *100.0 << std::endl;
        //     std::cout <<"Accuracy : "<< 100.0 - ((endToEndDist / trajSum) *100.0) << "%" << std::endl;                        
        // }
