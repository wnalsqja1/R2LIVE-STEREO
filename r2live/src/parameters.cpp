...
std::vector<std::string> CAM_NAMES; //from VINS Stereo 
...
Eigen::Vector3d G_gravity{0.0, 0.0, 1.0};
...
void readParameters(ros::NodeHandle &n)
{
    ...
    ROS_INFO("vinsfolderpath : %s",VINS_FOLDER_PATH);
    ...
    else
    {
        ...
        {
            cv::Mat cv_R, cv_T;
            fsSettings["extrinsicRotation_0"] >> cv_R;
            fsSettings["extrinsicTranslation_0"] >> cv_T;
        ...
        {
            cv::Mat cv_R, cv_T;
            fsSettings["extrinsicRotation_1"] >> cv_R;
            fsSettings["extrinsicTranslation_1"] >> cv_T;
            Eigen::Matrix3d eigen_R;
            Eigen::Vector3d eigen_T;
            cv::cv2eigen(cv_R, eigen_R);
            cv::cv2eigen(cv_T, eigen_T);
    ...
    /*same with VIN-FUSION and Original R2LIVE */
    INIT_DEPTH = 5.0;
