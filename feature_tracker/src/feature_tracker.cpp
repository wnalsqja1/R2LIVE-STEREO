...
// 추적 횟수가 많은 특징점 부터 마스크 적용하고, 255일 경우 특징점 유지하고, 특징점 주이 반경 Min_DIST 내의 모든 픽셀을 0으로 설정해 다른 특징점이 캡처하지 못하도록 설정 
...
void FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    if (prev_pts.size() > 0)
    {
        ...
        if(FLOW_BACK)
        {
            ...
            for(size_t i = 0; i < status.size(); i++)
            {
                if(status[i] && reverse_status[i] && distance(prev_pts[i], reverse_pts[i]) <= 1.0)
                {
                    status[i] = 1;
                }
                else
    ...
    if (PUB_THIS_FRAME)
    {
        // rejectWithF();
        TicToc t_m;
        setMask(); // 추적 횟수가 많은 특징점 부터 마스크 적용하고, 255일 경우 특징점 유지하고, 특징점 주이 반경 Min_DIST 내의 모든 픽셀을 0으로 설정해 다른 특징점이 캡처하지 못하도록 설정 
    ...
    if(!_img1.empty() && stereo_cam)
    {
        ...
        if(!cur_pts.empty())
        {
            ...
            if(FLOW_BACK)
            {
                ...
                for(size_t i = 0; i < status.size(); i++)
                {
                    if(status[i] && statusRightLeft[i] && inBorder(cur_right_pts[i]) && distance(cur_pts[i], reverseLeftPts[i]) <= 1){
                        status[i] = 1;
...
/*
    tracking feature points와 prior locations을 비교해 point 속도를 계산하는 함수.
    함수 cur_id_pts map<int,Point2f> 를 현재 프레임의 ID, locations으로 초기화, 이후 현재프레임-이전프레임 시간 차이를 계산해 point 속도를 계산.
    point 추적을 못했으면, 속도를 0으로 처리
*/
                               ...
                               map<int, cv::Point2f> &prevLeftPtsMap)
{
    ...
    for (size_t j = 0; j < curLeftPts.size(); j++)
    {
        ...
        cv::circle(imTrack, curLeftPts[j], 2, cv::Scalar(255 * (1 - len), 0, 255 * len), 2); //BGR :len 1 일 때, 갓 생성된게 빨간색 인 것 같은데 ?
    }
    ...
    if (!imRight.empty() && stereo_cam)
    {
        for (size_t i = 0; i < curRightPts.size(); i++)
        {
            ...
            rightPt.x += cols;
            cv::circle(imTrack, rightPt, 2, cv::Scalar(0, 255, 0), 2); //green
...
void FeatureTracker::readIntrinsicParameter(vector<string>& calib_file)
{
